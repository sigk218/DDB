...
komoran = Komoran()
skipwords = [
  '동물병원', '병원', '의사', '구름', '반려', '곳', '애견', '견', '원장', 
  '선생님', '간호사', '직원', '스태프', '개', '강아지','고양이', '여러분',
  '것', '거', '대체', '외', '너무', '때', '같은', '그런지', '안', '게', '모두', '년', '하루', '여자',
  '넘', '진짜', '도담이', 
]
umi = {
...
while t < len(ko_text):
  if ko_text.iloc[t, 0] == curr_hos:
    ...
    a_words, n_words = [], []
    for a in hos_a_reviews:
      a_words += komoran_tokenize(a)
    for n in hos_n_reviews:
      n_words += komoran_tokenize(n)
